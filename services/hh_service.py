...
class HHService:
    def __init__(self, config: HHConfig = None, secure_config_service: SecureConfigService = None):
        self.config = config
        self._secure_config_service = secure_config_service
    ...
    async def _fetch_page(self, session: aiohttp.ClientSession, params: Dict, page: int) -> Optional[Dict]:
        ...
        try:
            ...
            async with session.get(f"{self.api_base_url}/vacancies", params=clean_params) as response:
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.get(
                    f"{self.api_base_url}/suggests/areas",
    ...
    async def search_vacancies_with_user_auth(self, search_filter: Any, user_id: int) -> List[Dict]:
        """Поиск вакансий с авторизацией пользователя"""
        # Ленивый импорт ВНУТРИ метода
        if self._secure_config_service is None:
            return await self.search_vacancies(search_filter)
        
        # Получаем конфиг пользователя
        user_config = await self._secure_config_service.get_hh_config_for_user(user_id)
        if not user_config:
            logger.error(f"Для пользователя {user_id} нет HH конфигурации")
            return await self.search_vacancies(search_filter)  # Пробуем без авторизации
        
        # Используем конфиг пользователя
        original_config = self.config
        self.config = user_config
        
        try:
            result = await self.search_vacancies(search_filter)
            return result
        finally:
    ...
    async def check_vacancy_archived(self, vacancy_id: str) -> bool:
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.get(f"{self.api_base_url}/vacancies/{vacancy_id}") as response:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession(timeout=10) as session:
                async with session.get(f"{self.api_base_url}/vacancies", params={'text': 'test'}) as response:
                    return response.status == 200
        except Exception:
            return False
    
    # services/hh_service.py - добавьте метод
    async def ensure_valid_token(self, user_id: int, secure_storage) -> Optional[str]:
        """Обеспечивает наличие валидного access_token для пользователя"""
        try:
            # Получаем сохраненные данные
            secrets = await secure_storage.get_user_secrets(user_id)
            access_token = secrets.get('hh_access_token')
            ...
            # Проверяем срок действия через user_repo
            from database.repository import UserRepository
            user_repo = UserRepository(self.session)  # Нужно передать сессию!
            user = await user_repo.get_user_by_id(user_id)
            
            if user and user.hh_token_expires:
                from datetime import datetime
                if datetime.utcnow() >= user.hh_token_expires:
                    ...
                    if refresh_token and client_id and client_secret:
                        ...
                        if token_data and 'access_token' in token_data:
                            ...
                            await secure_storage.encrypt_and_save(
            ...
            logger.error(f"Ошибка в ensure_valid_token: {e}")
            return None
