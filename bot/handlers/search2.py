...
async def search_and_save_vacancies(callback: CallbackQuery, provider: DependencyProvider):
    ...
    try:
        # ИМИТАЦИЯ ПРОГРЕССА - этап 1
        await status_msg.edit_text("🔍 Подготовка к поиску... 10%")
        await asyncio.sleep(1)
        
        # ИМИТАЦИЯ ПРОГРЕССА - этап 2
        await status_msg.edit_text("🔍 Подключаюсь к HH API... 20%")
        await asyncio.sleep(1)
        
        # ИМИТАЦИЯ ПРОГРЕССА - этап 3
        await status_msg.edit_text("🔍 Формирую поисковый запрос... 30%")
        await asyncio.sleep(1)
        
        # Получаем HH Service
        hh_service = provider.hh_service
        
        # ИМИТАЦИЯ ПРОГРЕССА - этап 4
        await status_msg.edit_text("🔍 Ищу вакансии... 40%")
        
        # Запускаем реальный поиск
        search_task = asyncio.create_task(hh_service.search_vacancies(active_filter))
        
        # Показываем прогресс во время поиска
        progress_steps = [
            ("🔍 Запрашиваю первую страницу...", 50),
            ("🔍 Анализирую результаты...", 60),
            ("🔍 Получаю дополнительные страницы...", 70),
            ("🔍 Обрабатываю данные...", 80),
        ]
        
        for progress_text, progress_percent in progress_steps:
            if search_task.done():
                break
        ...
        for i, vacancy_data in enumerate(all_vacancies):
            try:
                ...
                # Обновляем статус каждые 5 вакансий или каждые 25%
                if (i + 1) % 5 == 0 or (i + 1) == total_count:
                    progress = 85 + int((i + 1) * 15 / total_count)
                    await status_msg.edit_text(
                        f"💾 Сохранение: {progress}%\n"
                        f"✅ Новых: {saved_count}\n"
