...
from .models import CoverLetter, GeneratedResume, User, SearchFilter, Vacancy, UserVacancy, LLMSettings
...
class UserRepository:
    ...
    async def update_user_scheduler_settings(self, telegram_id: str, scheduler_enabled: bool = None, scheduler_times: str = None, check_interval_hours: int = None) -> Optional[User]:
...
# Добавить в конец файла после StatisticsRepository

class GeneratedResumeRepository:
    def __init__(self, session: AsyncSession):
        self.session = session
    
    async def get_by_user_and_vacancy(self, user_id: int, vacancy_id: int) -> Optional[GeneratedResume]:
        """Получить сгенерированное резюме по пользователю и вакансии"""
    ...
    async def create(self, user_id: int, vacancy_id: int, title: str, content: str) -> GeneratedResume:
        """Создать новое сгенерированное резюме"""
        from .models import GeneratedResume
    ...
    async def update_hh_status(self, resume_id: int, hh_resume_id: str, is_uploaded: bool = True):
        """Обновить статус загрузки на HH"""
        from .models import GeneratedResume
        await self.session.execute(
...
class CoverLetterRepository:
    ...
    async def get_by_user_and_vacancy(self, user_id: int, vacancy_id: int) -> Optional[CoverLetter]:
        """Получить сопроводительное письмо по пользователю и вакансии"""
    ...
    async def create(self, user_id: int, vacancy_id: int, title: str, content: str) -> CoverLetter:
        """Создать новое сопроводительное письмо"""
        from .models import CoverLetter
        letter = CoverLetter(
            user_id=user_id,
            vacancy_id=vacancy_id,
...
# Обновить DependencyProvider в core/dependencies.py
# Добавить:
# self._generated_resume_repo = Generated